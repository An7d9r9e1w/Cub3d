...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/08/17 15:38:29 by nnamor            #+#    #+#             */
/*   Updated: 2021/08/30 13:52:26 by nnamor           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	valid_line(char *uline, char *line, char *dline, int *lens)
{
	int	len;
	int	i;
	...
	while (++i < len)
	{
		if (line[i] != '1' && line[i] != ' '
			&& (line[i - 1] == ' ' || line[i + 1] == ' '
				|| i > lens[0] || uline[i - 1] == ' '
				|| i >= lens[0] || uline[i] == ' '
				|| (i + 1) >= lens[0] || uline[i + 1] == ' '
				|| i > lens[2] || dline[i - 1] == ' '
				|| i >= lens[2] || dline[i] == ' '
				|| (i + 1) >= lens[2] || dline[i + 1] == ' '))
			return (1);
	}
	return (0);
}
...
int	valid_field(t_map *map, int player_count)
{
	...
	while (++i < field->size)
	{
		...
		if (valid_line(field->arr[i - 2], field->arr[i - 1],
				field->arr[i], len))
			return (EUCLSFIELD);
	}
	return (0);
